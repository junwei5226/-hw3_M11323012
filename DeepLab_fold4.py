...
Original file is located at
    https://colab.research.google.com/drive/1_ZfH-rjD7snFT0AAccWaTTmSNxnlj-6n
"""
...
# 建立結果目錄
!mkdir -p /content/results/fold4

# 將Fold4從本地上傳到Colab
print("請上傳Fold4資料夾的ZIP檔案...")
uploaded = files.upload()  # 上傳Fold4壓縮檔
...
# 檢查目錄結構
print("檢查Fold4目錄結構...")
if os.path.exists('/content/Fold4'):
    print("Fold4目錄內容:", os.listdir('/content/Fold4'))
else:
    print("找不到Fold4目錄，請確認解壓縮是否成功")
...
# 僅載入圖像函數
def load_images_no_mask(img_dir, img_size=(256, 256)):
    """僅載入圖像而不需要對應的遮罩"""
    images = []
    img_files = sorted(os.listdir(img_dir))
...
def data_augmentation(image, mask):
    ...
    if np.random.random() > 0.5:
        ...
        # 執行縮放 - 移除通道維度進行處理
        if len(original_shape) > 2:  # 如果有通道維度
            image_for_resize = image[:,:,0]
            mask_for_resize = mask[:,:,0]
        else:
            image_for_resize = image
            mask_for_resize = mask

        # 執行縮放
        image_resized = cv2.resize(image_for_resize, (new_w, new_h), interpolation=cv2.INTER_LINEAR)
        mask_resized = cv2.resize(mask_for_resize, (new_w, new_h), interpolation=cv2.INTER_NEAREST)
        ...
        if new_h < h or new_w < w:
            ...
            if len(original_shape) > 2:
                new_image = np.zeros((h, w, original_shape[2]))
                new_mask = np.zeros((h, w, original_shape[2]))
            else:
                new_image = np.zeros((h, w))
                new_mask = np.zeros((h, w))

            if len(original_shape) > 2:
                new_image[pad_h:pad_h+new_h, pad_w:pad_w+new_w, :] = image_resized
                new_mask[pad_h:pad_h+new_h, pad_w:pad_w+new_w, :] = mask_resized
            else:
                new_image[pad_h:pad_h+new_h, pad_w:pad_w+new_w] = image_resized
                new_mask[pad_h:pad_h+new_h, pad_w:pad_w+new_w] = mask_resized
...
# 載入訓練圖像 (無標註)
print("載入訓練圖像 (無標註)...")
train_images_no_annot = load_images_no_mask(train_img_path)
print(f"訓練圖像 (無標註): {train_images_no_annot.shape}")

# 處理缺少訓練標註和驗證資料的情況 - 使用測試資料分割
print("由於缺少訓練標註和驗證資料，將測試資料分割為訓練和驗證集...")
train_split_images, val_images, train_split_masks, val_masks = train_test_split(
    test_images, test_masks, test_size=0.2, random_state=SEED
)

# 更新訓練資料，用於後續流程
train_images = train_split_images
train_masks = train_split_masks

print(f"分割後的訓練集: {train_images.shape} - {train_masks.shape}")
print(f"分割後的驗證集: {val_images.shape} - {val_masks.shape}")
...
def build_deeplabv3_plus_model(input_shape=(256, 256, 1)):
    ...
    # 編譯模型 - 結合Dice Loss和Focal Loss (參考Fold2)
    model.compile(
        optimizer=keras.optimizers.Adam(learning_rate=1e-4),
        loss=[dice_loss, binary_focal_loss],
        loss_weights=[0.7, 0.3],  # 設定損失權重
        metrics=[iou_score]
    )
...
checkpoint = ModelCheckpoint(
    '/content/results/fold4/best_model.h5',
...
tensorboard = TensorBoard(
    log_dir='/content/results/fold4/logs',
    write_images=True
)
...
results_df.to_csv('/content/results/fold4/metrics.csv', index=False)
print("測試集評估結果:")
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Validation'], loc='upper left')
plt.savefig('/content/results/fold4/training_history.png')
plt.show()
...
def visualize_results(images, masks, predictions, num_samples=5):
    ...
    plt.tight_layout()
    plt.savefig('/content/results/fold4/predictions.png')
    plt.show()
...
# 壓縮並下載結果
!zip -r /content/fold4_results.zip /content/results/fold4
files.download('/content/fold4_results.zip')
